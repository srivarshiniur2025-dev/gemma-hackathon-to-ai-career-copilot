...
import { getMockTest, questionsForTest } from "@/lib/neet/catalog";
import { saveMockAttempt } from "@/lib/neet/progress";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";

export function MockRunner({ testId }: { testId: string }) {
  const test = getMockTest(testId);
  const [runId, setRunId] = useState(0);
  const questions = useMemo(
    () => (test ? questionsForTest({ ...test, id: `${test.id}-r${runId}` }) : []),
    [test, runId]
  );
  ...
  useEffect(() => {
    ...
    saveMockAttempt(test, score, questions.length);
  }, [done, test, questions, picked]);

  if (!test) {
    return (
      <div className="mx-auto max-w-lg py-16 text-center">
        <p className="text-muted">That mock could not be found.</p>
        <Link href="/mocks" className="mt-4 inline-block text-sm font-semibold text-accent">
          Back to mocks
        </Link>
      </div>
    );
  ...
  if (done) {
    ...
    return (
      <div className="mx-auto max-w-3xl space-y-6">
        <Link href="/mocks" className="inline-flex items-center gap-1 text-sm text-accent hover:underline">
        ...
        <div className="rounded-[28px] border border-border bg-white p-8 text-center shadow-[var(--shadow-lg)]">
          ...
          <h1 className="mt-2 font-heading text-3xl font-bold">
            {percent >= 70 ? "Strong paper." : percent >= 40 ? "Keep grinding." : "Review and retry."}
          </h1>
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            ...
            <Link href="/mocks">
              <Button variant="outline">Next mock</Button>
            </Link>
          </div>
        </div>
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <Link href="/mocks" className="inline-flex items-center gap-1 text-sm text-muted hover:text-accent">
        ...
        {q.options.map((opt, oi) => {
          ...
          return (
            <button
              ...
              className={cn(
                "w-full cursor-pointer rounded-[18px] border px-4 py-3 text-left text-sm transition-colors duration-200",
                selected
